...
@section('content')
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="table-responsive">
                        <table class="table table-striped custom-table datatable" id="categoryList" style="width: 100%">
                            <thead>
                                <tr>
                                    <th hidden>ID</th> 
                                    <th>No</th>
                            ...
                            <tbody>
                                @foreach ($categoryview as $key => $items)
                                    <tr>
                                        ...
                                        <td class="category_status">
                                            <span class="badge badge-large
                                                @if($items->category_status == 'Active') 
                                                    bg-inverse-primary 
                                                @elseif($items->category_status == 'InActive') 
                                                    bg-inverse-danger
                                                @endif">
                                                {{$items->category_status}}
                                            </span>
                                        </td>
                                        <td>{{ $items->category_creator }}</td>
                                        <td>
                                            <div class="dropdown dropdown-action">
                                                <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="material-icons">more_vert</i></a>
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    <a class="dropdown-item categoryUpdate" data-toggle="modal" data-id="{{ $items->id }}" data-target="#edit_category"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                                    <a class="dropdown-item categoryDelete" href="#" data-toggle="modal" data-target="#delete_category                            "><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div class="modal-body">
                        <form action="{{ route('form/update') }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <select class="form-control custom-select" id="catstatus_edit" name="category_status" required>
    ...
    <script>
        $(document).on('click', '.categoryUpdate', function() {
            ...
            $('#catname_edit').val(_this.find('.category_name').text().trim());
            $('#catstatus_edit').val(_this.find('.category_status span').text().trim());
        });
    </script>
